...
void dm4310_enable(motor_t *motor)
{
	...
	{
		case 0:
			enable_motor_mode(motor->id, MIT_MODE);		// 启用MIT模式
			break;
		case 1:
			enable_motor_mode(motor->id, POS_MODE);		// 启用位置模式
			break;
		case 2:
			enable_motor_mode(motor->id, SPEED_MODE);	// 启用速度模式
			break;
	}	
}
/**
...
void dm4310_fbdata(motor_t *motor, uint8_t *rx_data)
{
	...
	motor->para.p_int=(rx_data[1]<<8)|rx_data[2];				//位置整数
	motor->para.v_int=(rx_data[3]<<4)|(rx_data[4]>>4);			//速度整数
	motor->para.t_int=((rx_data[4]&0xF)<<8)|rx_data[5];			//扭矩整数
...
void mit_ctrl(uint16_t motor_id, float pos, float vel,float kp, float kd, float torq)
{
	...
	data[3] = (uint8_t)((vel_tmp&0xF)<<4)|(kp_tmp>>8);
...
void pos_speed_ctrl(uint16_t motor_id, float pos, float vel)
{
	...
	data[0] = *pbuf;				// 位置给定值字节1
	data[1] = *(pbuf+1);			// 位置给定值字节2
	data[2] = *(pbuf+2);			// 位置给定值字节3
	data[3] = *(pbuf+3);			// 位置给定值字节4

	data[4] = *vbuf;				// 速度给定值字节1
	data[5] = *(vbuf+1);			// 速度给定值字节2
	data[6] = *(vbuf+2);			// 速度给定值字节3
	data[7] = *(vbuf+3);			// 速度给定值字节4
